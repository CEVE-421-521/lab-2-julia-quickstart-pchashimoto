...
author: "Patricia Hashimoto pch3" # YOU SHOULD EDIT THIS
date: 2024-01-19
week: 2
categories: [Lab]
...
function read_tides(year::Int)
    ...
    df[!, " Water Level"] .*= 1u"m" # uses Unitful package
...
plot(
    df_month.datetime,
    df_month.lsl;
    title="Hourly Water levels in March 2020",
    ylabel="Water level",
    label=false,
)
```
...
dropmissing!(df, :lsl)
df_byyear = groupby(df, :year)
df_climatology = combine(df_byyear, :lsl => mean => :lsl_avg);
...
dropmissing!(df, :lsl)
df_bydayofyear = groupby(df, :dayofyear)
df_climatology = combine(df_bydayofyear, :lsl => mean => :lsl_avg);

plot(
    df_climatology.dayofyear,
    df_climatology.lsl_avg;
    xticks=([1,32,60,91,121,152,182,213,244,274,305,335], ["January", "February", "March", "April", "June", "July", "August", "September", "October", "November", "December"]),
    xlabel="Day of Year",
    ylabel="Average Water level",
    linewidth=3,
    label=false,
...
There appear to be seasonal trends in average water level, with the water level reaching its highest in late fall or early winter and also reaching a smaller peak in July.